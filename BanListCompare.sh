...
then
		#Public free node: unrestricted RPC is loopback-only and needs the RPC login.
			PRINT_WPL="$(./monero/build/release/bin/monerod --rpc-bind-ip=127.0.0.1 --rpc-bind-port=$MONERO_PUBLIC_PORT --rpc-login=${RPCu}:${RPCp} --rpc-ssl disabled print_pl | awk '$1=="white" {print $3}')" && echo "$PRINT_WPL" > /home/pinodexmr/WPeerList.txt;
			PRINT_GPL="$(./monero/build/release/bin/monerod --rpc-bind-ip=127.0.0.1 --rpc-bind-port=$MONERO_PUBLIC_PORT --rpc-login=${RPCu}:${RPCp} --rpc-ssl disabled print_pl | awk '$1=="gray" {print $3}')" && echo "$PRINT_GPL" > /home/pinodexmr/GPeerList.txt;
			PRINT_CP="$(curl -su ${RPCu}:${RPCp} --digest -X POST http://127.0.0.1:${MONERO_PUBLIC_PORT}/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_connections"}' -H 'Content-Type: application/json' | jq -Mr '.result.connections[] | .address," "')" && echo "$PRINT_CP" > /home/pinodexmr/CPeerList.txt
