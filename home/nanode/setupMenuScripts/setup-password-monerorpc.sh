...
whiptail --clear
    ...
    # ask user for new RPCp (second time)
		whiptail --title "Nanode RPC config" --passwordbox "Re-Enter Password" 16 45 2>$_temp
    ...
    if [ "${NEWRPCp1}" != "${NEWRPCp2}" ]; then
      whiptail --title "Nanode RPC Password" --msgbox "FAIL -> Passwords don't match\nPlease try again ..." 16 45
    ...
    if [ ${#NEWRPCp1} -eq 0 ]; then
      whiptail --title "Nanode - RPC Password" --msgbox "FAIL -> Password cannot be empty\nPlease try again ..." 16 45
	  ./setup-password-monerorpc.sh
      exit 1
    fi
    ...
    if [ ${#clearedResult} != ${#NEWRPCp1} ] || [ ${#clearedResult} -eq 0 ]; then
      whiptail --title "Nanode RPC Password" --msgbox "FAIL -> Contains bad characters (spaces, special characters)\nPlease try again ..." 16 45
    ./setup-password-monerorpc.sh
      exit 1
    fi
    ...
    if [ ${#NEWRPCp1} -lt 8 ]; then
      whiptail \
	  --title "Nanode RPC Password" --msgbox "FAIL -> Password length under 8\nPlease try again ..." 16 45
      ./setup-password-monerorpc.sh
      exit 1
    fi
...
#RPC Username - set
	whiptail --title "Nanode RPC config" --inputbox "New Username:" 10 60 2>$_temp
