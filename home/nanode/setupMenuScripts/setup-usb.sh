...
. /home/nanode/setupMenuScripts/setup-usb-path.sh
...
function fn_mountDrive () {
		...
		sudo mount -t $FILESYSTEM -o rw $fullDrivePath /home/nanode/.bitmonero
		sleep 2
}
...
function fn_addUuid () {
		...
		sudo sed "$ a UUID=$UUID /home/nanode/.bitmonero $FILESYSTEM noexec,defaults,nofail 0 2" -i /etc/fstab # '$' add to last line of file
}

	##Set permissions of mounted /.bitmonero dir
function fn_setPermissions () {
		sudo chown -R nanode /home/nanode/.bitmonero
		sudo chmod 777 -R /home/nanode/.bitmonero
}
...
function fn_createExt4 () {
			...
			echo "Mounting Device...Please Wait..."
			sudo mount -t ext4 -o rw $fullDrivePath /home/nanode/.bitmonero
			sleep 2
}
...
function fn_createNtfs () {
			...
			echo "Mounting Device...Please Wait..."
			sleep 2
			sudo mount -t ntfs -o rw $fullDrivePath /home/nanode/.bitmonero
			sleep 2
}
