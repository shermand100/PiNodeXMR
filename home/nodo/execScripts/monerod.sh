...
location=/home/nodo/monero/build/Linux/"$(getvar "versions.monero")"/release/bin
"$location"/monerod "$i2p_args" "$tor_args" --sync-mode="$SYNC_MODE" --data-dir="$DATA_DIR" --zmq-pub "tcp://$DEVICE_IP:18083" --confirm-external-bind "$rpc_args" --in-peers="$IN_PEERS" --out-peers="$OUT_PEERS" --limit-rate-up="$LIMIT_RATE_UP" --limit-rate-down="$LIMIT_RATE_DOWN" --max-log-file-size=10485760 --log-level=0 --max-log-files=1 --pidfile /run/monerod.pid --enable-dns-blocklist --detach
