...
. /home/nodo/common.sh
		...
		#WIDTH=60
		#CHOICE_HEIGHT=8
		CHOICE=$(whiptail --backtitle "Welcome" --title "Nodo Settings" --menu "\n\nWhat would you like to configure?" 20 60 10 \
...
case $CHOICE in
		...
		"2)")CHOICE2=$(whiptail --backtitle "Welcome" --title "Nodo Settings" --menu "\n\nSystem Settings" 20 60 10 \
				...
				case $CHOICE2 in
					...
					"2)") 	if (whiptail --title "Nodo Set Password" --yesno "This will change your SSH/Web terminal log in password\n\nWould you like to continue?" 12 78); then
					. /home/nodo/setupMenuScripts/setup-password-master.sh
							else
							fi
					;;

					"3)") 	if (whiptail --title "Nodo Set Password" --yesno "This will set your credentials needed to connect your wallet to your node\n\nWould you like to continue?" 12 78); then
					. /home/nodo/setupMenuScripts/setup-password-monerorpc.sh
							else
							fi
					;;

					"4)")	if (whiptail --title "Nodo configure storage" --yesno "This will allow you to add USB storage for the Monero blockchain.\n\nConnect your device now.\n\nWould you like to continue?" 16 78); then
					. /home/nodo/setupMenuScripts/setup-usb-select-device.sh
							else
							fi
					;;
					#Section 5 "Nodo Support Scripts" created to give ability of giving support to a user that requires complex or unique commands that they may not be able to action themselves. 3 scripts will be available, with default script action of no changes. User will be directed to activate the relevent script to assist them.
					"5)")	if (whiptail --title "Nodo Support Scripts" --yesno "This section allows for personalised support to be given to a Nodo user. Only continue if directed to by this project dev. \n\nWould you like to continue?" 16 78); then
					CHOICE2a=$(whiptail --backtitle "Welcome" --title "Nodo Settings" --menu "\n\nSystem Settings" 20 60 10 \
					...
					"1)")	if (whiptail --title "Nodo Support Scripts" --yesno "This will download and run the Nodo support script #1 from https://raw.githubusercontent.com/monero-ecosystem/PiNode-XMR/ubuntuServer-20.04/supportScript1.sh\n\nWould you like to continue?" 12 78); then
					wget -O - https://raw.githubusercontent.com/monero-ecosystem/PiNode-XMR/ubuntuServer-20.04/supportScript1.sh | bash
							else
							fi
					;;

					"2)") 	if (whiptail --title "Nodo Support Scripts" --yesno "This will download and run the Nodo support script #2 from https://raw.githubusercontent.com/monero-ecosystem/PiNode-XMR/ubuntuServer-20.04/supportScript2.sh\n\nWould you like to continue?" 12 78); then
					wget -O - https://raw.githubusercontent.com/monero-ecosystem/PiNode-XMR/ubuntuServer-20.04/supportScript2.sh | bash
							else
							fi
					;;

					"3)") 	if (whiptail --title "Nodo Support Scripts" --yesno "This will download and run the Nodo support script #3 from https://raw.githubusercontent.com/monero-ecosystem/PiNode-XMR/ubuntuServer-20.04/supportScript3.sh\n\nWould you like to continue?" 12 78); then
					...
					"6)")	if (whiptail --title "ATS : by tuxd3v | https://github.com/tuxd3v/ats#credits" --yesno "This utility will test the PWM fan plugged directly into the Nodo. This tool will read system temps and attempt to adjust fan speed accordingly\n\nWould you like to continue?" 16 78); then
					sudo apt-get update
		...
		"3)")CHOICE3=$(whiptail --backtitle "Welcome" --title "Nodo Settings" --menu "\n\nUpdate Tools" 20 60 10 \
				"1)" "Update Monero" \
				"2)" "Update Nodo" \
				...
				case $CHOICE3 in
					...
					"1)")	if (whiptail --title "Nodo Update Monero" --yesno "This will run a check to see if a Monero update is available\n\nIf an update is found Nodo will perform the update.\n\n***This will take several hours***\n\nWould you like to continue?" 12 78); then
							NEW_VERSION="$(curl -s https://raw.githubusercontent.com/monero-ecosystem/MoneroNodo/master/xmr-new-ver.txt)"
							...
							sleep "3"
								if [ $CURRENT_VERSION -lt $NEW_VERSION ]; then
									if (whiptail --title "Nodo Update Monero" --yesno "A Monero update has been found for your Nodo. To continue will install it now.\n\nWould you like to Continue?" 12 78); then
					...
					"2)")	if (whiptail --title "Update Nodo" --yesno "This will check for updates to PiNode-XMR Including performance, features and web interface\n\nWould you like to continue?" 12 78); then
							NEW_VERSION_PI="$(curl -s https://raw.githubusercontent.com/monero-ecosystem/MoneroNodo/master/new-ver-pi.txt)"
							...
							sleep "3"
								if [ $CURRENT_VERSION_PI -lt $NEW_VERSION_PI ]; then
									if (whiptail --title "Nodo Update" --yesno "An update has been found for your Nodo. To continue will install it now.\n\nWould you like to Continue?" 12 78); then

									wget -O - https://raw.githubusercontent.com/monero-ecosystem/PiNode-XMR/ubuntuServer-20.04/update-nodo.sh | bash -s "$NEW_VERSION_PI"

									else
									fi

								else
									if (whiptail --title "Nodo Update" --yesno "This device thinks it's running the latest version of Nodo.\n\nIf you think this is incorrect you may force an update below.\n\n*Note that a force update can also be used as a reset tool if you think your version is not functioning properly" --yes-button "Force PiNode-XMR Update" --no-button "Return to Main Menu"  14 78); then
					...
					"3)")	if (whiptail --title "Update Onion-Blockchain-Explorer" --yesno "This will check for and install updates to your Blockchain Explorer\n\nIf updates are found they will be installed\n\nWould you like to continue?" 12 78); then
						NEW_VERSION_EXP=$(curl -s https://raw.githubusercontent.com/monero-ecosystem/MoneroNodo/master/exp-new-ver.txt)
					...
					"6)")	if (whiptail --title "Update System" --yesno "Nodo will perform a check for background system updates of your OS's packages and dependencies.\n\nWould you like to continue?" 12 78); then
							clear;
		...
		"4)")CHOICE4=$(whiptail --backtitle "Welcome" --title "Nodo Settings" --menu "\n\nNode Tools" 20 60 10 \
				...
				case $CHOICE4 in
							"1)") . /home/nodo/setupMenuScripts/setup-explorer.sh	#Has functional legacy script, will change this format one day.
								;;

							"2)")	if (whiptail --title "Nodo Pop Blocks" --yesno "If you have errors for duplicate transactions in your log file, you may be able to 'pop' off the last blocks from the chain to un-freeze the sync process without syncing from scratch\n\nStop your Monero node before performing this action\n\nWould you like to continue?" 14 78); then
									. /home/nodo/setupMenuScripts/pop-blocks.sh
									else
									fi
								;;
							"3)")	if (whiptail --title "Nodo Clear Peer List" --yesno "There are occassions where the group of nodes you are connected to incorrectly report the top block height or otherwise prevent normal node operation. This option deletes p2pstate.bin and will allow Monero to build a fresh peer list on next node start.\n\nStop your Monero node before performing this action\n\nWould you like to continue?" 14 78); then
							...
							"4)") if (whiptail --title "Monero-LWS Install" --yesno "This will install the functional but developing Monero-LWS service\nOnce complete, the generated SSL certificate will also need installing on your wallet device\n\nWould you like to continue?" 14 78); then
									...
									. /home/nodo/setupMenuScripts/antelleGenrateIpCert.sh $(hostname -I)
									...
									. /home/nodo/variables/IPforLWS.sh
									#Install complete
								...
								;;
								"5)") if (whiptail --title "Nodo Monero-LWS Admin" --yesno "This tool is for adding your wallet address and view key so Monero-LWS can scan for your transactions in the background.\n\nMonero-LWS must be installed first\n\nWould you like to continue?" 14 78); then
								# use temp file
								_temp="./dialog.$$"
								#Wallet Address - set
								...
								/home/nodo/monero-lws/build/src/monero-lws-admin add_account $walletadd $viewkey
								...
								/home/nodo/monero-lws/build/src/monero-lws-admin rescan 0 $walletadd
								else
									. /home/nodo/setup.sh
									fi
								;;
		...
		"5)")CHOICE5=$(whiptail --backtitle "Welcome" --title "PiNode-XMR Settings" --menu "\n\nExtra Network Tools" 30 60 15 \
				...
				case $CHOICE5 in
							...
							"1)")	if (whiptail --title "Nodo Install tor" --yesno "Some countries, for censorship, political or legal reasons do not look favorably on tor and other anonymity services, so tor is not installed on this device by default. However this option can install it now for you.\n\nWould you like to continue?" 14 78); then
									. /home/nodo/setupMenuScripts/setup-tor.sh
									else
									fi
								;;

							"2)")	if (whiptail --title "Nodo tor NYX" --yesno "This tool will allow you to monitor tor bandwidth usage\n\nWhen prompted for a password, enter 'Nodo'\nAnd to exit the utility press 'CTRL+C' \n\nWould you like to continue?" 12 78); then
									nyx
									else
									fi
								;;

							"3)")	if (whiptail --title "Nodo Start/Stop tor" --yesno "Manually Start or Stop the service." --yes-button "Start tor" --no-button "Stop tor"  14 78); then
							...
							"4)")	if (whiptail --title "Nodo Install I2P" --yesno "This will install the I2P server/router onto your Nodo\n\nWould you like to continue?" 14 78); then
									. /home/nodo/setupMenuScripts/setup-i2p.sh
									else
									fi
								;;

							"5)")	if (whiptail --title "Nodo Start/Stop I2P" --yesno "Manually Start or Stop the service." --yes-button "Start I2P" --no-button "Stop I2P"  14 78); then
									i2prouter start;
							...
							"6)")	if (whiptail --title "Nodo PiVPN Install" --yesno "This feature will install PiVPN on your Nodo\n\nPiVPN is a simple to configure openVPN server.\n\nFor more info see https://pivpn.dev/\n\nWould you like to continue?" 12 78); then
									. /home/nodo/setupMenuScripts/setup-PiVPN.sh
									else
									fi
								;;

							"7)")	CHOICE6=$(whiptail --backtitle "Nodo Settings" --title "Web Interface Password Set/Enable/Disable" --menu "\n\nWeb Interface Password Set/Enable/Disable" 30 60 15 \
								...
								"1)")	if (whiptail --title "Web Interface Password Set" --yesno "Set the password used to access the Web Interface" --yes-button "Set Password" --no-button "Cancel" 14 78); then
									sudo htpasswd -c /etc/apache2/.htpasswd nodo
									else
									fi
								;;
								...
								"2)")	if (whiptail --title "Web Interface Password Enable" --yesno "This will enable the requirement for password authentication to access the Web Interface\n\nWould you like to continue?" 12 78); then
									sudo cp /home/nodo/variables/000-default-passwordAuthEnabled.conf /etc/apache2/sites-enabled/000-default.conf
								...
								"3)")	if (whiptail --title "Web Interface Password Disable" --yesno "This will disable the requirement for password authentication to access the Web Interface\n\nWould you like to continue?" 12 78); then
									sudo cp /home/nodo/variables/000-default-passwordAuthDisabled.conf /etc/apache2/sites-enabled/000-default.conf
									sudo chown root /etc/apache2/sites-enabled/000-default.conf
									sudo chmod 777 /etc/apache2/sites-enabled/000-default.conf
									sudo systemctl restart apache2
									#Update htmlPasswordRequired flag for use with Nodo updater script
							...
							"8)")	if (whiptail --title "Nodo Configure Dynamic DNS" --yesno "This will configure Dynamic DNS from NoIP.com\n\nFirst create a free account with them and have your username and password before continuing\n\nWould you like to continue?" 12 78); then
									. /home/nodo/setupMenuScripts/setup-noip.sh
									else
									fi
								;;
