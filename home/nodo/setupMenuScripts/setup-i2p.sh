...
sleep 1
sudo mv /home/nodo/i2p.list /etc/apt/sources.list.d/
...
###Make web console accessable on lan
	...
	#WEB_CLIENT_PORT="$(cat /home/nodo/.i2p/clients.config.d/00-net.i2p.router.web.RouterConsoleRunner-clients.config | sed '3q;d' | awk '{ print $1, $4 }' | cut -d'=' -f2-)"
	...
	sed -i "/clientApp.0.args*/c\clientApp.0.args=7657 ::1,127.0.0.1,"${DEVICE_IP}" ./webapps/" /home/nodo/.i2p/clients.config.d/00-net.i2p.router.web.RouterConsoleRunner-clients.config
	#This allows access from hostname e.g: http://nodo.local:7657
	echo "routerconsole.allowedHosts=nodo.local" >> /home/nodo/.i2p/router.config
