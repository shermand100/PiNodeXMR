...
then	
		#Node Status
		PRINT_TX_SHORT="$(./monero/build/release/bin/monerod --rpc-bind-ip=127.0.0.1 --rpc-bind-port=18081 --rpc-login=${RPCu}:${RPCp} --rpc-ssl disabled print_pool_sh | sed '1d')" && echo "$PRINT_TX_SHORT" > /var/www/html/TXPool-short_Status.txt
fi
