...
then	
		#Node Status
		PRINT_TX_SHORT="$(./monero/build/release/bin/monerod --rpc-bind-ip=$DEVICE_IP --rpc-bind-port=18081 --rpc-login=${RPCu}:${RPCp} --rpc-ssl disabled print_pool_sh | sed '1d')" && echo "$PRINT_TX_SHORT" > /var/www/html/TXPool-short_Status.txt
fi
	
	if [ $BOOT_STATUS -eq 6 ]
then
		#Adapted command for restricted public rpc calls (payments)
		PRINT_TX_SHORT="$(./monero/build/release/bin/monerod --rpc-bind-ip=$DEVICE_IP --rpc-bind-port=$MONERO_STATS_PORT --rpc-ssl disabled print_pool_sh | sed '1d')" && echo "$PRINT_TX_SHORT" > /var/www/html/TXPool-short_Status.txt
fi

	if [ $BOOT_STATUS -eq 7 ]
then
		#Adapted command for public free (restricted) rpc calls. No auth needed for local.
		PRINT_TX_SHORT="$(./monero/build/release/bin/monerod --rpc-bind-ip=$DEVICE_IP --rpc-bind-port=$MONERO_PORT --rpc-ssl disabled print_pool_sh | sed '1d')" && echo "$PRINT_TX_SHORT" > /var/www/html/TXPool-short_Status.txt
fi
