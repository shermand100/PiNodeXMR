...
then	
		#Node Status
		PRINT_POOL_STATS="$(./monero/build/release/bin/monerod --rpc-bind-ip=$DEVICE_IP --rpc-bind-port=18081 --rpc-login=${RPCu}:${RPCp} --rpc-ssl disabled print_pool_stats | sed '1d')" && echo "$PRINT_POOL_STATS" > /var/www/html/TXPool_Status.txt
fi	
