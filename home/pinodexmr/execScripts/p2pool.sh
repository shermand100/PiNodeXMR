...
if [ $BOOT_STATUS -eq 3 ] || [ $BOOT_STATUS -eq 5 ]
		...
		./p2pool --host $DEVICE_IP --rpc-port $MONERO_PORT --rpc-login $RPCu:$RPCp --wallet $MINING_ADDRESS --data-api /var/www/html/api/ --local-api --no-color --light-mode --no-cache --loglevel 3 --data-dir ~/p2pool.log --mini
fi
...
if [ $BOOT_STATUS -eq 6 ]
		...
		./p2pool --host $DEVICE_IP --rpc-port $MONERO_PORT --wallet $MINING_ADDRESS --data-api /var/www/html/api/ --local-api --no-color --light-mode --no-cache --loglevel 3 --data-dir ~/p2pool.log --mini
fi
...
if [ $BOOT_STATUS -eq 7 ]
		...
		./p2pool --host $DEVICE_IP --rpc-port $MONERO_PUBLIC_PORT --wallet $MINING_ADDRESS --data-api /var/www/html/api/ --local-api --no-color --light-mode --no-cache --loglevel 3 --data-dir ~/p2pool.log --mini
fi
