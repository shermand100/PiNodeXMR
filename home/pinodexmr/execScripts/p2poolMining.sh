...
if [ $BOOT_STATUS -eq 3 ] || [ $BOOT_STATUS -eq 5 ]
		...
		./p2pool --host $DEVICE_IP --rpc-port $MONERO_PORT --rpc-login $RPCu:$RPCp --wallet $MINING_ADDRESS --start-mining $MINING_THREADS --data-api /var/www/html/api/ --local-api --out-peers $OUT_PEERS_P2POOL --in-peers $IN_PEERS_P2POOL --no-color --light-mode --no-cache --loglevel 1 --data-dir /home/pinodexmr/p2pool/build/ $P2POOLCHAIN
fi
...
if [ $BOOT_STATUS -eq 6 ]
		...
		./p2pool --host $DEVICE_IP --rpc-port $MONERO_PORT --wallet $MINING_ADDRESS --start-mining $MINING_THREADS --data-api /var/www/html/api/ --local-api --out-peers $OUT_PEERS_P2POOL --in-peers $IN_PEERS_P2POOL --no-color --light-mode --no-cache --loglevel 1 --data-dir /home/pinodexmr/p2pool/build/ $P2POOLCHAIN
fi
...
if [ $BOOT_STATUS -eq 7 ]
		...
		./p2pool --host $DEVICE_IP --rpc-port $MONERO_PUBLIC_PORT --wallet $MINING_ADDRESS --start-mining $MINING_THREADS --data-api /var/www/html/api/ --local-api --out-peers $OUT_PEERS_P2POOL --in-peers $IN_PEERS_P2POOL --no-color --light-mode --no-cache --loglevel 1 --data-dir /home/pinodexmr/p2pool/build/ $P2POOLCHAIN
fi
