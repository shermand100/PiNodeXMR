...
#Import Start Flag Values:
	...
	#Import tx-proxy port
	. /home/pinodexmr/i2p-tx-proxy-port.sh
	#Import ADD_PEER (seed) port
	. /home/pinodexmr/add-peer.sh
...
DNS_PUBLIC=tcp ./monerod --p2p-bind-ip 127.0.0.1 --rpc-bind-ip=127.0.0.1 --rpc-bind-port=$I2P_PORT --rpc-login=$RPCu:$RPCp --confirm-external-bind --anonymous-inbound $I2P_ADDRESS:$I2P_PORT,$DEVICE_IP:18080 --tx-proxy i2p,127.0.0.1:$I2P_TX_PROXY_PORT --rpc-ssl disabled --log-level=1 --add-peer $ADD_PEER --in-peers=$IN_PEERS --out-peers=$OUT_PEERS --limit-rate-up=$LIMIT_RATE_UP --limit-rate-down=$LIMIT_RATE_DOWN --log-file=/var/www/html/monerod.log --max-log-file-size=10485000 --max-log-files=1 --pidfile /home/pinodexmr/monero/build/release/bin/monerod.pid --detach