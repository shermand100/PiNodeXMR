...
then
		...
		#Node Version Print
		VERSION="$(./monero/build/release/bin/monerod --rpc-bind-ip=${DEVICE_IP} --rpc-bind-port=${MONERO_PORT} --rpc-login=${RPCu}:${RPCp} --rpc-ssl disabled version | sed -n '2p')" && echo "${VERSION#*I}" | cut -c12- > /var/www/html/node_version.txt
