...
#If drive hold correct label
	...
	"2)" "Don't recover. Re-format the drive to start again." 2>&1 >/dev/tty)
		...
		sudo sed -i '3d' /etc/fstab #removes existing entry if this script run before (delete 3rd line fstab)
		sudo sed "2 a UUID=$UUID /home/pinodexmr/.bitmonero $FILESYSTEM noexec,defaults,nofail 0 2" -i /etc/fstab
