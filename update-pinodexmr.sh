#!/bin/bash
	...
	if (whiptail --title "PiNode-XMR Updater" --yesno "This will update PiNode-XMR to the newest version\n\nContinue?" 12 78); then
		...
		#Backup User values (rename all variables that are user-set by appending with '_retain'. These will then later overwrite the freshly downloaded ones to resore user values.)
